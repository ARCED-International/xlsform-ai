...
def build_column_mapping(worksheet, header_row: int, required_columns: list = None) -> dict:
    """
    Scan header row to build dynamic column name to position mapping.

    This function solves the critical bug where hardcoded column positions
    cause data to be written to wrong columns when Excel files have different
    column ordering than expected.

    Args:
        worksheet: openpyxl worksheet object
        header_row: Row number containing column headers
        required_columns: List of column names that must exist (default: ['type', 'name', 'label'])

    Returns:
        Dict mapping lowercase column names to their 1-based positions

    Raises:
        ValueError: If required columns are missing from header

    Example:
        >>> ws = wb["survey"]
        >>> header = find_header_row(ws)
        >>> columns = build_column_mapping(ws, header)
        >>> type_col = columns['type']  # Actual position of 'type' column
        >>> constraint_col = columns.get('constraint', None)  # None if not present
    """
    ...
    for col_idx in range(1, max_col + 1):
        ...
        if cell_value:
            column_name = str(cell_value).strip().lower()
    ...
    if missing:
        raise ValueError(
            f"Missing required columns in XLSForm: {', '.join(missing)}\n"
            f"Please ensure your XLSForm has these columns in the survey sheet."
        )

    return column_mapping


def find_choices_insertion_point(worksheet, header_row: int = 1) -> int:
    """
    Find first empty row in choices sheet (scans from top).

    More reliable than max_row + 1 which may return artificially high values
    due to formatting in empty rows.

    Args:
        worksheet: openpyxl worksheet object (choices sheet)
        header_row: Header row number (default: 1)

    Returns:
        Row number where new choices should be inserted

    Example:
        >>> ws_choices = wb["choices"]
        >>> insert_row = find_choices_insertion_point(ws_choices)
        >>> # insert_row will be 2 if sheet is empty (just header)
        >>> # insert_row will be 5 if there are 3 choices (rows 2, 3, 4)
    """
    # Build column mapping
    try:
        column_mapping = build_column_mapping(worksheet, header_row)
    except ValueError:
        # Fallback if no valid headers
        column_mapping = {'list_name': 1, 'name': 2, 'label': 3}

    # Key columns that must ALL be empty for a row to be considered empty
    key_columns = ['list_name', 'name', 'label']

    # Start scanning from row after header
    for row_idx in range(header_row + 1, worksheet.max_row + 2):
        is_empty = True

        # Check if all key columns are empty
        for col_name in key_columns:
            if col_name in column_mapping:
                cell_value = worksheet.cell(row_idx, column_mapping[col_name]).value
    ...
    # If all rows have data, append at end
